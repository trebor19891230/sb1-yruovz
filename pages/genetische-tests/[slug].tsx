...
interface GeneticTestingPageProps {
  content: SEOContent;
  relatedContent: SEOContent[];
}

export default function GeneticTestingPage({ content, relatedContent }: GeneticTestingPageProps) {
...
export const getStaticPaths: GetStaticPaths = async () => {
  const paths = getAllSEOSlugs().filter(slug => slug.startsWith('/genetische-tests/'));
  return {
    paths,
    fallback: false,
  };
